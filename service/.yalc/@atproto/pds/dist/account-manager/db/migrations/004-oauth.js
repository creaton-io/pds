...
async function up(db) {
    ...
    await db.schema
        ...
        .expression((0, kysely_1.sql) `code DESC) WHERE (code IS NOT NULL`)
        .execute();
    await db.schema
    ...
    await db.schema
        ...
        .addForeignKeyConstraint('device_account_device_id_fk', ['deviceId'], 'device', ['id'], (qb) => qb.onDelete('cascade').onUpdate('cascade'))
        .execute();
    await db.schema
        .createTable('token')
    ...
    await db.schema
        ...
        .addForeignKeyConstraint('used_refresh_token_fk', ['tokenId'], 'token', ['id'], 
...
//# sourceMappingURL=004-oauth.js.map